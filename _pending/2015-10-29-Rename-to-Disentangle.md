...
The above differentiates Disentangle from its predecessors. Graphs are fall in a gap between universal structures like lists and purpose-built structures like a Fibonacci-heap-cache-backed-by-a-stream or [master-and-servant queues of AtomicMarkedReferences](). [Graphs have a lot of variation and have a lot of commonality.]() Other graph frameworks require developers to begin by supplying graphs defined in that framework's structures. Developers who attempt to use those frameworks frequently give up when the framework doesn't meet their needs. They either rip out the old work (or worse leave it gumming up the works) and replace it with something exactly custom to their own needs. 
...
Instead of calling allPairsShortestPaths() call allPairsLeastPaths() to generate all the shortest paths.
    ...
    lazy val leastPathLabels: Seq[(String, String, support.Label)] = Dijkstra.allPairsLeastPaths(edges,
                                                                                                support,
                                                                                                support.convertEdgeToLabel[String](stringToDouble))


Because Disentangle is semiring-based, it's possible to create your own semirings to drive the algorithms. For example, here's one for [most probable paths](). 

Try it out. Pull it into a project via


Or clone a project that lets you play in the console

>
> cd disentangleExample
> sbt console

---

I think something lighter is the answer. The API for algorithms in Disentangle 

More complex allows for custom semirings 
 
The graph structures it uses are available and I think pretty good. Minimal -- everything is used by algorithm code.
 
Try it out.



My hope is that people working with graphs will interpret a name like Disentangle as a hopeful. 

Why not ScalaGraphMinimzer
Disentangle
* Simple, non-invasive API
* Non-invasive - Uses some of Scala's common currencies - Collections and Tuples - as its API
* Not a framework or a domain-specific language
** Frameworks have to provide more
** DSLs don't match directed graphs well in the real world. Collections of tuples do.
